...
**Carry both attribution lines through verbatim** —
`Generated by Qase Quality Supervisor · analyzing-change-impact skill`, in the header
`.sub` line and in the footer. They are the plugin's attribution; a filled copy that drops them is wrong even
if every number in it is right.
...
3. **Mark it as AI-generated**, tied to branch, commit and a UTC timestamp:
   `_Generated by Qase Quality Supervisor · analyzing-change-impact skill · branch <name> · commit <sha> · <timestamp> — review before relying on it._`
