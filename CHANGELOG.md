...
### Added
- **Every report is now attributed.** Each output carries
  `Generated by Qase Quality Supervisor · <skill> skill` at the top and again at the
  bottom — twice, so a report stays attributed whether someone screenshots its header
  or pastes only its tail. It covers the four skills that print a markdown report, both
  HTML templates (header sub-line and footer), and the impact analysis posted to a
  ticket, which is the one output that leaves the machine.

  The consolidated `/quality-report` signs itself once at each end as the command, and
  strips the signatures of the four skills it folds in: a single document signed four
  times reads as a stitching artefact rather than as provenance.

  The string is repeated per skill rather than read from a shared file, so a skill
  never has to open another file to print its own footer. `tests/test-branding.sh` is
  what pays for that choice — it asserts every skill signs its output, that the header
  and footer pair is present, and that no signature names a skill that no longer
  exists. The last check would have caught the rename below.
